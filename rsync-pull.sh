...
rsync -aq --delete -f"+ */" -f"- *" ${DATASRV}:${RSDIR}/* ${BACKUPDIR} 2>/dev/null | ts '[rsync pull] '
RRC=$?
if [ -t 0 ]; then # if the script is not run by cron
...
for DIRPATH in ${RSDIRS}; do
  ...
  rsync -aq --no-recursive ${DATASRV}:${RSDIR}/${DIRPATH}/* ${BACKUPDIR}/${DIRPATH} 2>/dev/null | ts '[rsync pull] '
  RRC=$?
  if [ -t 0 ]; then # if the script is not run by cron
    if [[ "${RRC}" = "0" ]]; then
